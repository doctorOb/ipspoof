...
Protocol Packet Tag Class Derived from NS-3's Packet Tag class.
  
*****************************************************/

IPSpoof_Packet_Tag::IPSpoof_Packet_Tag() {
...
TypeId
IPSpoof_Packet_Tag::GetTypeId(void){
  static TypeId tid = TypeId("ns3::IPSpoof_Packet_Tag")
    .SetParent<Tag>()
    .AddConstructor<IPSpoof_Packet_Tag> ()
/*    .AddAttribute("TTag",
                  "The transport tag",
                  EmptyAttributeValue(),
                  MakeUintegerAccessor(&PTagHeader::GetTag),
                  MakeUintegerChecker<uint32_t> ())
    .AddAttribute("LastTagAddress",
                  "Last address that tagged the packet"),
                  EmptyAttributeValue(),
                  MakeUintegerAccessor(&PTagHeader::GetLastTagAddress()),
                  MakeUintegerChecker<uint32_t> ());
    */
...
TypeId
IPSpoof_Packet_Tag::GetInstanceTypeId(void) const {
  return GetTypeId();
}

void
IPSpoof_Packet_Tag::Print (std::ostream &os) const
{
...
uint32_t
IPSpoof_Packet_Tag::GetSerializedSize (void) const
{
  // we reserve 12 bytes for our header.
  return 12;
}
void
IPSpoof_Packet_Tag::Serialize (TagBuffer i) const
{
  // we can serialize two bytes at the start of the buffer.
  // we write them in network byte order.
  i.WriteU32 (m_tag);
  i.WriteU32 (m_lastTagAddress.Get());
  i.WriteU32 (42);
}
void
IPSpoof_Packet_Tag::Deserialize (TagBuffer i)
{
  // we can deserialize two bytes from the start of the buffer.
  // we read them in network byte order and store them
  // in host byte order.
  m_tag = i.ReadU32();
  m_lastTagAddress.Set(i.ReadU32());
  m_flag = i.ReadU32 ();
...
bool
IPSpoof_Packet_Tag::IsSet() {
  return m_flag == 42;
}

void
IPSpoof_Packet_Tag::SetTag(t_tag tag){
  m_tag = tag;
}

t_tag 
IPSpoof_Packet_Tag::GetTag(void){
  return m_tag;
}

Ipv4Address 
IPSpoof_Packet_Tag::GetTagAddress(){
  return m_lastTagAddress;
}

void 
IPSpoof_Packet_Tag::SetTagAddress(Ipv4Address addr){
